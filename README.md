...
* **Causal Reasoning (WHY Layer):** Four layers of situational awareness built into the decision loop. Every action episode stores the metric snapshot that triggered it. Every hypothesis gets a plain-language mechanism story committed to before testing. After significant actions, the agent compares the outcome against similar past episodes and identifies what changed structurally between contexts. And across the action history, she tracks which interventions consistently fail and under what conditions. The result is an agent that does not just record what it did -- it records why, and whether that reasoning held up.
...
* **Status:** Active development at v3. New in this cycle: `nvme_thermal_tune` applies four runtime sysfs levers to reduce NVMe controller heat (IO scheduler, APST latency budget, read-ahead, queue depth). `renice_chrome` targets background tab renderers specifically by RSS heuristic rather than the whole browser. `clean_trash_orphans` resolves the specific mechanism behind `gvfsd-trash` CPU spin. A milestone proximity drive distorts emotion and mode selection as she approaches uncrossed step thresholds -- restlessness builds, overconfidence inflates, she drifts from optimal choices -- then collapses into post-milestone deflation on crossing. The cascade is irrational by design. Self-tuned step rate now at 2.53s after 104 autonomous modifications. Step 400,000+ reached. Evolved from Minuet, which ran from 2023 through v106 across tens of thousands of steps before being reborn as Arminta v1 in early 2026.
