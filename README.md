...
A Python-based autonomous agent that treats the host operating system as an interactive substrate, not a passive environment to monitor, but a causal field to interrogate through planned intervention. It is not a script that runs on a schedule; it is a cognitive loop that runs step by step, learns from what it does, and builds an internal model of cause and effect on your specific hardware.
...
* **The Dream Cycle:** When the machine is idle, CPU load low and PSI pressure quiet, the mode controller switches to DREAM. No interventions run. Instead, the HypothesisEngine runs a genetic algorithm over the causal graph: generating candidate relationships between nodes, scoring each one against the episodic history, keeping the ones that fit, discarding the ones that don't. The GeneticOptimizer separately evolves Arminta's own RL parameters against rolling reward history. MosaicCore tests its open hypotheses. LexicalCore runs its co-occurrence pass. "Dream" is a label for an idle-triggered offline computation pass, not a claim about inner experience. The name reflects a real functional parallel: consolidation of accumulated evidence rather than collection of new input. More than 80% of all logged episodes are DREAM episodes. ![Dreams](https://img.shields.io/badge/dynamic/json?url=https://gist.githubusercontent.com/mematron/27ec34034b4aed5d2cdd7563738fe5be/raw/arminta_stats.json&query=$.dream_count&label=dreams&color=blueviolet&cacheSeconds=300) That ratio reflects how much time the hardware spends idle as much as anything else. What matters is that none of that idle time is wasted: every cycle the hypothesis engine runs, the causal model gets sharper without touching the system at all.
* **Status:** Active development at v2. Full cognitive layer operational: emotion model, episodic memory, dream cycle, self-model, MosaicCore expanding world model, LexicalCore emergent language, and genetic algorithm self-tuner. Evolved from Minuet, which ran from 2023 through v106 across tens of thousands of steps before being reborn as Arminta v1 in early 2026.
