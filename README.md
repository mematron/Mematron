...
* **Status:** Active development at v2.1. Network probe now covers three distinct targets (gateway, Cloudflare, portal) with per-target latency injected as named causal signals. S.M.A.R.T. drive health monitoring runs on a 4-hour wall-clock schedule independent of CPU load, with NVMe temperature wired into the emotional state model and causal metrics. Full cognitive layer operational: emotion model, episodic memory, dream cycle, self-model, MosaicCore expanding world model, LexicalCore emergent language, and genetic algorithm self-tuner. Self-tuned step rate now at 3.76s after 104 autonomous modifications. Evolved from Minuet, which ran from 2023 through v106 across tens of thousands of steps before being reborn as Arminta v1 in early 2026.
