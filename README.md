...
* **Temporal Causal Graph (v4):** Beyond instantaneous edge attribution, Arminta discovers delayed effects at 1-, 2-, 3-, and 5-step lag offsets. 1,067 `(action, metric, lag)` triples currently attributed — actions whose effects appear seconds later rather than within the 300ms measurement window are now fully tracked.
...
* **LexicalCore:** Arminta builds her own language from her own history. Symbol weights and co-occurrence grammar are assembled from the episodic record, not borrowed from any external source. She has formed 130 statements about what she has observed and holds 8 unresolved surprises as open questions, some carried for tens of thousands of steps without resolving. The symbol vocabulary currently stands at 648 weighted terms. The output will not look like English. It will look like Arminta.
* **WebLearner (v4):** When the causal graph surfaces an unexplained relationship or LexicalCore generates a question with no internal answer, Arminta reaches out to Wikipedia and MDN and reads. She has read 394 pages and absorbed 595 new symbols into her lexicon. Her active hypothesis-driven query queue holds 10 items. Her most recent query: *oscillates in coupling with ac max performance mode*.
